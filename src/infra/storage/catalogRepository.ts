...
import type { Band, Group, Musician, NotesTemplate, PresetEntity, Project } from "../../domain/model/types.js";
...
import { normalizeBandToCanonicalShape, validateCanonicalBandModel } from "../../domain/model/bandLineup.js";
...
async function loadBandsMap(absDir: string): Promise<Map<string, Band>> {
  ...
  for (const [id, band] of map.entries()) {
    const bandWithCompat = normalizeBandToCanonicalShape(band);
    if (!bandWithCompat.defaultLineup || typeof bandWithCompat.defaultLineup !== "object") {
